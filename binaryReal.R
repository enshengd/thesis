...
# c("ld","dtree","fda","mda","knn","log","svm","ptree","neural","nb","C50","pls")
# c("knn","ld","log","svm","dtree","ptree","neural","nb","C50","pls")
# c("knn","log","svm","ptree","neural","nb","C50","pls")
# c("ld","dtree","fda","mda")
# c("knn","ld","log","svm","neural","nb","C50","pls")
...
for (id in c(221)) {
  ...
  outputFileA<-paste('/Users/ensheng/OneDrive/stat500/results/nsim100/',date,sep="")
  outputFileB<-paste(outputFileA,'_',sep="")
  outputFileC<-paste(outputFileB,toString(fileNum),sep="")
  outputFileD<-paste(outputFileC,'_',sep="")
  outputFileE<-paste(outputFileD,fileName,sep="")
  outputFile<-paste(outputFileE,'.txt',sep="")
  ...
  ############ Real Data Set ############
  realData0<-realData[realData[,1]==0,][,-1]
  ...
  for (models in modelList) {
    ...
    for (k in sigma.noise) {
      ...
      #rownames(roc.final)<-noisy.repl
      #colnames(roc.final)<-noisy.train
      print ("roc results: ")
      print (roc.final)
      
      # roc difference
      rocdiff.final<-(rocdiff.sum/nsim)[noisy.repl,noisy.train]  # an element of roc.diff.mean
      #rownames(rocdiff.final)<-noisy.repl
      #colnames(rocdiff.final)<-noisy.train
      ...
      #rownames(kl.final)<-noisy.repl
      #colnames(kl.final)<-noisy.train
      print ("kl results: ")
      print (kl.final)
      
      # kl difference
      kldiff.final<-(kldiff.sum/nsim)[noisy.repl,noisy.train]
      #rownames(kldiff.final)<-noisy.repl
      #colnames(kldiff.final)<-noisy.train
      print ("kl difference: ")
      print (kldiff.final)
      ...
      #rownames(eu.final)<-noisy.repl
      #colnames(eu.final)<-noisy.train
      print ("eu results: ")
      print (eu.final)
      
      # eu difference
      eudiff.final<-(eudiff.sum/nsim)[noisy.repl,noisy.train]
      #rownames(eudiff.final)<-noisy.repl
      #colnames(eudiff.final)<-noisy.train
      print ("eu difference: ")
      print (eudiff.final)
  ...
  #save the images
  imageROC1<-paste(outputFileE,"_roc",sep="")
  imageROC<-paste(imageROC1,".png",sep="")
  png(imageROC, width=12, height=8, units="in", res=300)
  multiplot(roc.p1, roc.p2, roc.p3, roc.p4, roc.p8, roc.p9, roc.p10, roc.p12, cols=4)
  ...
  # multiplot(roc.p2, roc.p5, roc.p11, roc.p13, cols=4)
  dev.off()

  imageKL1<-paste(outputFileE,"_kl",sep="")
  imageKL<-paste(imageKL1,".png",sep="")
  png(imageKL, width=12, height=8, units="in", res=300)
  multiplot(kl.p1, kl.p2, kl.p3, kl.p4, kl.p8, kl.p9, kl.p10, kl.p12, cols=4)
  ...
  # multiplot(kl.p2, kl.p5, kl.p11, kl.p13, cols=4)
  dev.off()

  imageRMSE1<-paste(outputFileE,"_rmse",sep="")
  imageRMSE<-paste(imageRMSE1,".png",sep="")
  png(imageRMSE, width=12, height=8, units="in", res=300)
  multiplot(eu.p1, eu.p2, eu.p3, eu.p4, eu.p8, eu.p9, eu.p10, eu.p12, cols=4)
  ...
  # multiplot(eu.p2, eu.p5, eu.p11, eu.p13, cols=4)
  dev.off()
  dev.off()
